...
void GeodeticLine::genGeometry() {
    ...
    for (unsigned int i = 0; i < noOfVds4D; i++) {
...
float WorldLine::intersect(const Intersectable& intersectable) {
    float t;
    if (intersectable.getType() == RelTypes::IntersectionMode::lightCone) {
        t = intersectLightCone(reinterpret_cast<const LightCone&>(intersectable));
    }
    else if (intersectable.getType() == RelTypes::IntersectionMode::hyperplane) {
        t = intersectHyperplane(reinterpret_cast<const Hyperplane&>(intersectable));
    }
    return t;
}

int WorldLine::getNoOfVds() {
    return noOfVds4D;
}

std::vector<vec4>& WorldLine::getVds() {
    return vds4D;
}

const int WorldLine::getNoOfVds() const {
    return noOfVds4D;
}

const std::vector<vec4>& WorldLine::getVds() const {
    return vds4D;
}
...
vec4 GeodeticLine::getClosestLocation(const Ray& ray, const RelTypes::ObserverProperties& observerProperties, const RelTypes::Settings& settings)
{
...
vec4 GeodeticLine::getLocationAtZeroT() {
    return locationAtZeroT;
}

vec4 GeodeticLine::getVelocity() {
    return fourVelocity;
}
...
CompositeLine::CompositeLine(std::vector<vec4>& points, std::string _name, std::string _desc)
    : WorldLine(_name, _desc) {
    controlPoints = points;
    genGeometry();
}

CompositeLine::CompositeLine(vec3 _posAtZeroT, vec3 _velocity, std::string _name, std::string _desc)
    : WorldLine(_name, _desc) {
...
vec4 CompositeLine::getClosestLocation(const Ray& ray, 
    const RelTypes::ObserverProperties& observerProperties,
    const RelTypes::Settings& settings)
{
...
void CompositeLine::setControlPoints(std::vector<vec4>& points) {
    controlPoints = points;
}

std::vector<vec4>& CompositeLine::getControlPoints() {
...
SpiralLine::SpiralLine(vec3 _posAtZeroT, vec3 _centerOfRotation, vec3 _velocity, std::string _name, std::string _desc) : WorldLine(_name, _desc)
{
    ...
    fourVelocityAtZeroT = RelPhysics::ToFourVelocity(_velocity);
    noOfVds4D = 500;
    genGeometry();
}
...
void SpiralLine::draggedTo(vec4 location)
{
    //TODO
}

vec4 SpiralLine::getClosestLocation(const Ray& ray, const RelTypes::ObserverProperties& observerProperties, const RelTypes::Settings& settings)
{
    return vds4D[0];
}

void* SpiralLine::createView()
{
    return new SpiralLineView(this);
}

vec4 SpiralLine::getLocationAtZeroT()
{
    return vec4();
}
