...
vec4 Observer::getLocation(const RelTypes::Settings& settings)
{
...
vec4 Observer::getVelocity(const RelTypes::Settings& settings)
{
	return interpolate<bool, vec4>(settings.doLorentz, true, false,
...
vec4 Observer::getLocationAtZero(const RelTypes::Settings& settings)
{
...
RelTypes::ObserverProperties Observer::getProperties(const RelTypes::Settings& settings) {
...
Hyperplane* Observer::getHyperplane(const RelTypes::Settings& settings)
{
	vec4 location = getLocation(settings);
...
*/
LightCone* Observer::getLightCone(const RelTypes::Settings& settings)
{
	return new LightCone(getLocation(settings));
}

vec3 Observer::transformPosToDiagramSpace(vec4 absoluteFramePos4, const RelTypes::Settings& settings) {
...
inline vec3 Observer::transformNormalToDiagramSpace(vec4 absoluteFrameNormal4, const RelTypes::Settings& settings) {
...
vec3 Observer::transformConeAxisToDiagramSpace(const RelTypes::Settings& settings)
{
...
void Observer::drawHyperplane(GPUProgram& gpuProgram, const Camera& camera, const RelTypes::Settings& settings)
{
	...
	gpuProgram.setUniform(TranslateMatrix(pos), "getModellMatrix");
...
void Observer::setCurrentTimeAtAbsoluteTime(float t, const RelTypes::Settings& settings)
{
...
float Observer::getAbsoluteTimeAtCurrentTime(const RelTypes::Settings& settings)
{
	if (settings.doLorentz.get()) {
...
float Observer::changeTimeByDelta(float deltaTau, const RelTypes::Settings& settings)
{
...
void Observer::syncCamera(Camera* camera, const RelTypes::Settings& settings)
{
	camera->syncToObserver(getProperties(settings));
}

void Observer::syncTimeToObserversSimultaneity(Observer& observer, const RelTypes::Settings& settings)
{
...
void Observer::loadOnGPU(GPUProgram& gpuProgram, const RelTypes::Settings& settings)
{
...
vec4 Observer::getClosestLocation(const Ray& ray, const RelTypes::ObserverProperties& observerProperties, const RelTypes::Settings& settings)
{
